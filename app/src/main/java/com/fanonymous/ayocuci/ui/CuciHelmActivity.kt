...
class CuciHelmActivity : AppCompatActivity(), DatePickerDialog.OnDateSetListener,
    TimePickerDialog.OnTimeSetListener {

    @BindView(R.id.btn_back)
    lateinit var btnBack: ImageView

    @BindView(R.id.edt_jenis_helm)
    lateinit var edtJenisHelm: EditText
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.activity_cuci_helm)
    ...
    private fun setSpinner() {
        val jenisService = resources.getStringArray(R.array.jenis_service_helm)
    ...
    fun onBtnBookingClicked() {
        if (edtJenisHelm.text.trim().length > 0 && !btnTanggal.text.equals("Pilih Tanggal Booking") && pilihanJenisService.trim().length > 0) {
            AlertDialog.Builder(this@CuciHelmActivity)
                .setTitle("Booking Cuci Helm?")
                .setMessage("Yakinkah kamu ingin booking untuk cuci helm?")
            ...
            alert.showAlertDialog(this@CuciHelmActivity, "Booking gagal..", "Data masih ada yang kosong!!!", false)
        }
    }

    private fun booking() {
        baseResponseCall = apiInterface.bookingAdd(sessionManager.getIdUser()!!, "Helm", edtJenisHelm.text.toString(), pilihanJenisService, btnTanggal.text.toString())
        baseResponseCall.enqueue(object : Callback<BaseResponse> {
            override fun onResponse(call: Call<BaseResponse>, response: Response<BaseResponse>) {
                try {
                    pd.dismiss()
                    if (response.body()!!.msg.equals("Berhasil")) {
                        Toast.makeText(this@CuciHelmActivity, response.body()!!.msg, Toast.LENGTH_SHORT).show()
                        onBackPressed()
                    } else {
                        alert.showAlertDialog(this@CuciHelmActivity, "Booking gagal..", response.body()!!.msg, false)
                    }
                } catch (e: Exception) {
                    e.printStackTrace()
                }
            ...
            override fun onFailure(call: Call<BaseResponse>, t: Throwable) {
                pd.dismiss()
                alert.showAlertDialog(this@CuciHelmActivity, "Booking gagal..", Server.CHECK_INTERNET_CONNECTION, false)
            }
    ...
    fun onBtnTanggalClicked() {
        ...
        val datePickerDialog =
            DatePickerDialog(this@CuciHelmActivity, this@CuciHelmActivity, year, month, day)
    ...
    override fun onDateSet(view: DatePicker?, year: Int, month: Int, dayOfMonth: Int) {
        ...
        val timePickerDialog = TimePickerDialog(
            this@CuciHelmActivity, this@CuciHelmActivity, hour, minute,
