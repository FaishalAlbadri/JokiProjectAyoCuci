...
class CuciKarpetActivity : AppCompatActivity(), DatePickerDialog.OnDateSetListener,
    TimePickerDialog.OnTimeSetListener {

    @BindView(R.id.btn_back)
    lateinit var btnBack: ImageView

    @BindView(R.id.edt_jenis_karpet)
    lateinit var edtJenisKarpet: EditText
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.activity_cuci_karpet)
    ...
    private fun setSpinner() {
        val jenisService = resources.getStringArray(R.array.jenis_service_karpet)
    ...
    fun onBtnBookingClicked() {
        if (edtJenisKarpet.text.trim().length > 0 && !btnTanggal.text.equals("Pilih Tanggal Booking") && pilihanJenisService.trim().length > 0) {
            AlertDialog.Builder(this@CuciKarpetActivity)
                .setTitle("Booking Cuci Karpet?")
                .setMessage("Yakinkah kamu ingin booking untuk cuci karpet?")
            ...
            alert.showAlertDialog(this@CuciKarpetActivity, "Booking gagal..", "Data masih ada yang kosong!!!", false)
        }
    }

    private fun booking() {
        baseResponseCall = apiInterface.bookingAdd(sessionManager.getIdUser()!!, "Karpet", edtJenisKarpet.text.toString(), pilihanJenisService, btnTanggal.text.toString())
        baseResponseCall.enqueue(object : Callback<BaseResponse> {
            override fun onResponse(call: Call<BaseResponse>, response: Response<BaseResponse>) {
                try {
                    pd.dismiss()
                    if (response.body()!!.msg.equals("Berhasil")) {
                        Toast.makeText(this@CuciKarpetActivity, response.body()!!.msg, Toast.LENGTH_SHORT).show()
                        onBackPressed()
                    } else {
                        alert.showAlertDialog(this@CuciKarpetActivity, "Booking gagal..", response.body()!!.msg, false)
                    }
                } catch (e: Exception) {
                    e.printStackTrace()
                }
            ...
            override fun onFailure(call: Call<BaseResponse>, t: Throwable) {
                pd.dismiss()
                alert.showAlertDialog(this@CuciKarpetActivity, "Booking gagal..", Server.CHECK_INTERNET_CONNECTION, false)
            }
    ...
    fun onBtnTanggalClicked() {
        ...
        val datePickerDialog =
            DatePickerDialog(this@CuciKarpetActivity, this@CuciKarpetActivity, year, month, day)
    ...
    override fun onDateSet(view: DatePicker?, year: Int, month: Int, dayOfMonth: Int) {
        ...
        val timePickerDialog = TimePickerDialog(
            this@CuciKarpetActivity, this@CuciKarpetActivity, hour, minute,
